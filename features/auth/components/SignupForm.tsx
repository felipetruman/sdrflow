...
export function SignupForm() {
  const router = useRouter(); const [message, setMessage] = useState(''); const [serverError, setServerError] = useState('')
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({ resolver: zodResolver(schema) })
  const onSubmit = async ({ email, password }: FormValues) => { setServerError(''); setMessage(''); const result = await signUp({ email, password }); if (result?.error) return setServerError(result.error); setMessage('Conta criada. Redirecionando para o login...'); setTimeout(() => router.push('/login'), 800) }
