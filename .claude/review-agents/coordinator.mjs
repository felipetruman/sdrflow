...
function autoFixTrivial(deduped) {
  ...
  const isSafePath = (p) => {
    ...
    const abs = resolve(p);
    return abs.startsWith(cwd + "/") && !relative(cwd, abs).startsWith("..");
  };
  const fixable = deduped.filter((f) => f.auto_fixable && isSafePath(f.file) && existsSync(f.file));
  ...
  // Auto-commits removed: LLM-driven git writes are a supply-chain risk.
  // Findings are reported as PR comments only; humans apply fixes manually.
