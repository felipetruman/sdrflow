...
Esse schema cobre: workspaces, membros, funil, leads, campos personalizados, regras de campos obrigatórios, campanhas, mensagens geradas, histórico de atividades e RLS.
...
-- generated_messages (via subquery em leads)
...
### 9.8 Decisões do schema

1. **`value text` universal em `lead_custom_values`** — simplifica o MVP. Poderia evoluir para colunas tipadas.
2. **`generated_messages` sem `workspace_id`** — acesso controlado via RLS com subquery em `leads`.
3. **`source` em vez de `lead_source`** — nomenclatura mais curta.
4. **`status` no lead** — permite marcar como inativo/convertido/perdido sem mudar de etapa.
5. **`slug` no workspace** — prepara para URLs amigáveis no futuro.

---

### 9.9 Como defender essa modelagem

> Modelei o banco em torno de workspaces porque isolamento de dados era um requisito central. Cada entidade operacional carrega `workspace_id`, e o acesso é protegido por RLS. Separei campos personalizados em definição e valores para manter flexibilidade sem perder estrutura relacional. As mensagens geradas ficam persistidas para permitir histórico, regeneração e envio simulado. A função de criação de workspace evita inconsistência no onboarding e já cria o funil padrão.
