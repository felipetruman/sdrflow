...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch active keys for this workspace, ordered by primary then priority
    ...
    const inserts = messages.map((content) => ({
      ...
      status: 'generated' as const, generation_type: 'manual' as const,
    }))
    ...
    await supabase.from('lead_activities').insert({
      ...
      metadata: {
        campaign_id: campaignId, campaign_name: campaign.name, count: messages.length,
